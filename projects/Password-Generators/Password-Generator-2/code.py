...
def password_generator():
    ...
    try:
        ...
        length_of_special_characters = int(
            input("Enter the length of special characters: ")
        )
    except ValueError:
        print("Invalid Input!")
    else:
        ...
        passwordLength = (
            length_of_alphabets + length_of_digits + length_of_special_characters
        )
        # generate a password for user based on the total password length
        securePassword = "".join(
            secrets.choice(string.ascii_letters) for i in range(length_of_alphabets)
        )
        securePassword += "".join(
            secrets.choice(string.digits) for i in range(length_of_digits)
        )
        securePassword += "".join(
            secrets.choice(string.punctuation)
            for i in range(length_of_special_characters)
        )
        ...
        print(
            "Your password of length {} is {}".format(
                passwordLength, "".join(generated_password)
            )
        )
