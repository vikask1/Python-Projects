...
def Create():
    try:
        mycursor.execute(
            "create table bank(ACCNO varchar(10),NAME varchar(20),MOBILE varchar(10),EMAIL varchar(50),ADDRESS varchar(20),CITY varchar(10),COUNTRY varchar(20),BALANCE integer(15))"
        )
        print("Table Created")
        Insert()
    except:
...
def Insert():
    while True:
        ...
        new_acc = (
            dt_string
            + ", "
            + "Account Created, Available balance: Rs."
            + str(Bal)
            + "\n"
        )
...
def Debit():
    try:
        ...
        S = mycursor.fetchall()
        print(
            "Please Note that the money can only be debited if min balance of Rs 5000 exists "
        )
